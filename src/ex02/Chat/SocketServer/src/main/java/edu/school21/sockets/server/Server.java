    ...
    public final class Client {
        ...
        private void signIn() throws IOException {
            ...
            if (user == null) {
                ...
                this.user = user;
                System.err.println("Client signed in as " + user.getUsername());
                roomMenu();
            }
        }
        ...
        private void chooseRoom() throws IOException {
            String choice = "";
        do {
            List<Room> roomList = roomsService.listAllRooms();

            for (int i = 0; i < roomList.size(); i++)
                println((roomList.get(i).getIdentifier().toString() + " " + (roomList.get(i).getName())));
            choice  = readNonEmpty();
            for (int i = 0; i < roomList.size(); i++) {
                if (Long.getLong(choice) == roomList.get(i).getIdentifier()) {
                    roomsService.enterRoom(user ,Long.getLong(choice));
                    startMessaging();
                } else {
                    println("Wrong input!");
                }
